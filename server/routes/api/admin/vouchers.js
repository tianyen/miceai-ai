    ...
    try {
        ...
        const summary = await database.get(`
            SELECT
                ...
                SUM(total_quantity - remaining_quantity) as redeemed_quantity
            FROM vouchers
            WHERE is_active = 1
        `);
        ...
        const byCategory = await database.all(`
            ...
            WHERE is_active = 1
            GROUP BY category
            ORDER BY redeemed_quantity DESC
        `);

        // 3. 今日兌換統計（如果有提供日期）
        let todayRedemptions = null;
        if (date) {
            todayRedemptions = await database.all(`
                SELECT
                    v.voucher_name,
                    v.category,
